...
After applying the documented invalid-date correction proposal to the ignored corpus snapshot, the corrected module-local WarSampo baseline covers all 61 sources and 13,873,089 triples. It completed in 120,236 ms with a sampled peak JVM heap of 1,977.3 MiB and reported 123 violations plus one warning. The two source defects found by strict parsing—`1939-12-35` and `1940-02-30`—remain captured as a portable, reviewable patch rather than an undocumented corpus edit. The reviewed baseline v2 signatures are committed in `baselines/warsampo-local.tsv`; details and the reproducible command are recorded in [the corrected baseline record](../../baselines/2026-09-01-warsampo.md).

The first completed cross-module audit ran in 390,133 ms with a 2,369.0 MiB sampled heap peak. It added 1,643 violations and 379 warnings: chiefly systematic preferred-label conflicts between actor and casualty exports, unresolved internal IRIs, and untyped participant aliases. Per-rule timing localized and removed a combinatorial chronology join; the optimized rule completes in 1,742 ms on the full union. The [cross-module audit record](../../baselines/2026-09-01-warsampo-union.md) preserves the exact result and performance breakdown without treating the unreviewed findings as an accepted baseline.
...
Cross-module execution is implemented using TDB2, verified on fixtures, and now measured on the complete corpus. It remains an explicit audit path rather than a routine CI gate because a run takes approximately 6.5 minutes and its integration findings are not yet reviewed. The per-rule profile makes global audit-target versions of the two 1.7-million-focus-node label checks the next clear optimization if more frequent execution is required.
